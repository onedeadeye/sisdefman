...
class CliTests(unittest.TestCase):
    ...
    def test_series_commands(self):
        code, out = self.run_cli("series", "new", "crate2", "--first-id", "210", "--last-id", "296",
                                 "--name", "Second Series")
        self.assertEqual(code, 0, out)
        code, out = self.run_cli("series", "new", "clash", "--first-id", "150", "--last-id", "160", "--name", "C")
        self.assertEqual(code, 1)
        code, out = self.run_cli("series", "new", "unnamed", "--first-id", "300", "--last-id", "310")
        self.assertEqual(code, 1)
        self.assertIn("give the series a display name", out)
        code, out = self.run_cli("series", "new", "swallow", "--first-id", "100", "--last-id", "105", "--name", "S")
        self.assertEqual(code, 1)
